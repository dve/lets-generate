...
public class VaadinDataProviderGenerator extends AbstractCodeGenerator {
  ...
  public void writeCode(ProcessingEnvironment processingEnvironment, DataBeanModel model)
    ...
    TypeSpec dataProviderClass = TypeSpec.classBuilder(model.getName() + classSuffix())
        ...
        .superclass(ParameterizedTypeName.get(ClassName.get(SizeChangeAwareDataProvider.class),
            JPoetUtils.getBeanClassName(model), filterClassName(model)))
        .addField(JPoetUtils.getBaseQueriesClassName(model), BASE_QUERIES_FIELD, Modifier.PRIVATE)
